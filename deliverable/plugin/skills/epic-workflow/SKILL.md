...
**Ticket sizing:** one ticket = one future spec-workflow run — a short deliverable statable in a sentence or two. A phase that can't be stated that tightly gets split.
...
The open script finds or starts the playbook viewer; it exits silently if the viewer can't start — don't treat that as an error, fall back to showing the draft in chat before asking.
...
- **Show ticket seed** — print the ticket body formatted for pasting into `/playbook:spec-workflow`
...
- Don't deepen tickets toward spec plans. If a ticket needs acceptance criteria, it's time to run `/playbook:spec-workflow` on it, not to grow the ticket.
