...
- **Root AGENTS.md out of spec** → recommend `/playbook:agents-md-setup` (review mode); piecemeal patching fights that skill's interview.
- **Shipped ADR edited beyond a supersession banner** → show what changed (`git diff` against the adding commit) and recommend reverting or a superseding ADR via `/playbook:adr`. Rewriting records is not doctor's call.
