...
| `trace-plan/shared/authoring-rules.md` | A **synced copy** of `trace/shared/authoring-rules.md`. See the cross-plugin gotcha below |
...
- **A plugin cannot read files outside its own directory.** Claude Code copies each plugin into its own cache directory; paths that traverse out (`../other-plugin/shared/x.md`) resolve to nothing after install. Two consequences, both load-bearing:
  ...
  - **`authoring-rules.md` is physically duplicated** into `trace-plan/shared/`. `trace/shared/` is the source; `scripts/sync-shared.js` propagates it and the release preflight fails on drift. Adding another cross-boundary file means adding a row to that script's `TARGETS` — never hand-copy.
