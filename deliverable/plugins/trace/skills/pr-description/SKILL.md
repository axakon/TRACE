...
- **With a PR number** (`/trace:pr-description 123`): run `gh pr view 123 --json baseRefName,headRefName,number,title,body`, fetch the base if needed, and diff against `origin/<base>`.
- **With a base branch** (`/trace:pr-description main`): diff `<base>...HEAD`.
