...
Resolve the scope per [scope-resolution.md](../../shared/scope-resolution.md) — `$ARGUMENTS` if given, otherwise cwd. A root `AGENTS.md` is not required — the recommended order is `/trace:init` then `/trace:agents-md-setup`. Init does **not** touch any `AGENTS.md` outside the chosen folder; the directory index is `/trace:agents-md-setup`'s job.
