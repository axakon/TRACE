      ...
      res.on('end', () => {
        try {
          ...
          if (info.service === SERVICE) {
            return resolve({ status: 'ours', plansDir: info.plansDir, epicsDir: info.epicsDir, specCommand: info.specCommand });
          }
        } catch {}
...
async function findServer(dir, field = 'plansDir', specCommand) {
  const target = normDir(dir);
  let free = null;
  for (let i = 0; i < MAX_PORTS; i++) {
    const port = BASE_PORT + i;
    const result = await probe(port);
    if (result.status === 'ours' && normDir(result[field] || '') === target && (!specCommand || result.specCommand === specCommand)) {
      return { port, running: true };
    }
    if (result.status === 'free' && !free) free = { port, running: false };
  }
  return free;
}
      ...
      const cmd =
        process.platform === 'darwin'
          ...
          : process.platform === 'win32'
            ? ['cmd', ['/c', 'start', '', url]]
            : ['xdg-open', [url]];
      execFile(cmd[0], cmd[1], (error) => error ? reject(error) : resolve());
    });
  });
}
