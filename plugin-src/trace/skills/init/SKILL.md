---
name: init
description: Configure TRACE for this scope — choose where durable project context lives, copy the canonical three-category doc structure into it, mark the folder, and persist the choice. Recommended as the first TRACE command in a fresh repo, before {{skill:trace:agents-md-setup}}. Safe to re-run to change the location.
...
1. Choose where this scope's durable context lives (the folder `{{skill:trace:distil}}` writes into).
...
Resolve the scope per [scope-resolution.md](../../shared/scope-resolution.md) — `{{arguments}}` if given, otherwise cwd. A root `AGENTS.md` is not required — the recommended order is `{{skill:trace:init}}` then `{{skill:trace:agents-md-setup}}`. Init does **not** touch any `AGENTS.md` outside the chosen folder; the directory index is `{{skill:trace:agents-md-setup}}`'s job.
...
```
node "{{scripts}}/copy-doc-structure.js" <chosen-folder>
```
...
- `errors` — paths the script could not write (e.g. a file sitting where a directory is needed). Show each path and message to the developer and stop; they resolve the obstruction, then re-run `{{skill:trace:init}}` — re-running is safe, already-written files are skipped.
...
- If no `AGENTS.md` exists at the scope root, one line suggesting `{{skill:trace:agents-md-setup}}` next (pass the same path argument when the scope isn't cwd) — that skill owns the scope's AGENTS.md, and without it `{{skill:trace:distil}}` won't recognise this folder as its own scope.
...
- Both `{{skill:trace:distil}}` and `{{skill:trace:agents-md-setup}}` read this scope's config. If it is absent, both fall back to the precedence in [docs-folder-resolution.md](../../shared/docs-folder-resolution.md).
