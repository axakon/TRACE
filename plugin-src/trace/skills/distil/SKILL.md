...
`node "{{scripts}}/doctor.js" check`

Pre-existing findings unrelated to this run are not yours to fix here — mention `{{skill:trace:doctor}}` and move on.

Once this run reaches a clean conclusion — candidates written, or a no-op from Phase 2 — clear the sentinel:

`node "{{scripts}}/clear-sentinel.js"`
