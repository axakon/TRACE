---
name: scaffold-docs
description: Suggest and scaffold starter docs for a project's durable-context folder, tailored to what the repo evidences. One-time bootstrap on a project that has code but little documentation, after {{skill:trace:init}} and {{skill:trace:agents-md-setup}}.
...
Files land in `<docs-folder>/system/` (descriptive — what the codebase IS) and, for the one paired topic, also `<docs-folder>/architecture/` (prescriptive — what the system MUST do). The three-category model itself is described in the READMEs `{{skill:trace:init}}` copies into place; scaffold assumes those READMEs are there.
...
Resolve the scope's durable-context folder per [docs-folder-resolution.md](../../shared/docs-folder-resolution.md). The folder must contain `system/README.md` and `architecture/README.md` (placed by `{{skill:trace:init}}`). If either is missing, tell the developer to run `{{skill:trace:init}}` first (pass the same path argument when the scope isn't cwd) and stop — scaffold relies on the canonical structure being in place.
...
- `{{skill:trace:distil}}` keeps `system/` and `architecture/` files current as work happens.
...
- This is a one-time bootstrap. Ongoing capture is `{{skill:trace:distil}}`'s job.
