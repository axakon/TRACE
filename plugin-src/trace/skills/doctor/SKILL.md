...
```
node "{{scripts}}/doctor.js" check <scope-root>
```
...
2. **Renumber the other file** to the report's `suggested_free` number:
   ...
   ```
   node "{{scripts}}/doctor.js" migrate <file> <NNNN> <scope-root>
   ```
...
- **Missing canonical READMEs / structure** → run the copy directly: `node "{{scripts}}/copy-doc-structure.js" <docs-folder>` (copies only missing files; never overwrites).
...
- Other skills and agents can run the script directly as a cheap deterministic check — `node "{{scripts}}/doctor.js" check` — without invoking this skill's fix flow.
