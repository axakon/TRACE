...
Most commits look like the first one, a title and nothing else. The second adds a body because the title cannot carry the *why*. Only the third earns bullets, and it earns two, not five.
...
The first draft had four more bullets. The request limit, the 429 status, and the `Retry-After` header are all visible in the diff. The fourth repeated a comment above the counter.
