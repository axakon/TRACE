#!/usr/bin/env node
// Localhost HTTP server for the TRACE viewer: renders Claude Code plan
...
// Env:   TRACE_PLAN_VIEWER_PORT overrides the default port (PLAYBOOK_PLAN_VIEWER_PORT still honoured).
...
// Focus channel — lets the open hooks steer a tab that is already open instead
// of spawning another one. Every page polls GET /api/focus; a GET consumes the
// pending target, so exactly one tab navigates however many are open.
let pendingFocus = null;
let lastFocusPollAt = 0;
// Browsers throttle timers in hidden tabs to roughly once a minute, so a tab
// that has been quiet for a while is usually still open. 90s keeps it counted
// as live; past that the hook opens a new tab rather than navigating nothing.
const FOCUS_STALE_MS = 90000;

function parseArgs(argv) {
  const opts = {
    port: Number(process.env.TRACE_PLAN_VIEWER_PORT || process.env.PLAYBOOK_PLAN_VIEWER_PORT) || DEFAULT_PORT,
...
// --- Epics -----------------------------------------------------------------
// Tickets carry minimal YAML frontmatter per the epic skill contract:
...
// Claims the pending focus target, if any, and navigates there. Runs on every
// page so the open hooks can reuse this tab whatever it is currently showing.
// Polls on the same 1.5s cadence as the content refresh, and again the moment
// the tab becomes visible so a throttled background tab catches up instantly.
const FOCUS_SCRIPT = `
(() => {
  ...
  async function claim() {
    try {
      ...
      if (data && data.url && data.url !== here()) location.href = data.url;
    } catch {}
  }
...
function seedFor(epic, t) {
  return (
    'Use ${specCommand} on this ticket from epic "' + epic.title + '".\\n' +
...
function main() {
  ...
  async function route(req, res) {
    ...
    if (url.pathname === '/api/info') {
      return send(
        res,
        200,
        JSON.stringify({ service: 'trace-plan-viewer', plansDir, epicsDir, pid: process.pid, specCommand }),
        'application/json'
      );
    }

    // POST parks a target for an open tab to pick up and reports whether one is
    // listening; GET claims it. The hooks only park when `live` comes back true,
    // and open a browser themselves otherwise.
    if (url.pathname === '/api/focus') {
      if (req.method === 'POST') {
        let target = null;
        try {
          const parsed = JSON.parse(await readBody(req));
          if (typeof parsed.url === 'string' && parsed.url.startsWith('/')) target = parsed.url;
        } catch {}
        const live = Date.now() - lastFocusPollAt < FOCUS_STALE_MS;
        if (live && target) pendingFocus = target;
        return send(res, 200, JSON.stringify({ live }), 'application/json');
      }
      lastFocusPollAt = Date.now();
      const target = pendingFocus;
      pendingFocus = null;
      return send(res, 200, JSON.stringify({ url: target }), 'application/json');
    }
    ...
    // which read from <epicsDir>/.preview — the staging area the epic skill
  ...
  server.listen(port, '127.0.0.1', () => {
    console.log(
      `TRACE viewer serving plans:${plansDir} epics:${epicsDir} at http://127.0.0.1:${port}/`
    );
  });
