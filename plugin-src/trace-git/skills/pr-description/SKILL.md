...
- **With a PR number** (`{{skill:trace-git:pr-description}} 123`): run `gh pr view 123 --json baseRefName,headRefName,number,title,body`, fetch the base if needed, and diff against `origin/<base>`.
- **With a base branch** (`{{skill:trace-git:pr-description}} main`): diff `<base>...HEAD`.
...
Without `gh` or `glab`, the PR-number and apply paths are unavailable, and base inference falls back to git. For a single commit, use `{{skill:trace-git:commit-message}}`.
