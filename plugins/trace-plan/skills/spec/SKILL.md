...
If one might qualify, offer it using the question control: **Record an ADR** / **Skip**. On **Record an ADR**, load and follow the installed $trace:adr skill — it owns the full gate, the template, numbering, and the write, and has the planning decision in context. It may still decline the decision as too small; that's the correct outcome. Don't write the ADR yourself. On **Skip**, continue.
