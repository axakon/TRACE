...
Files land in `<docs-folder>/system/` (descriptive — what the codebase IS) and, for the one paired topic, also `<docs-folder>/architecture/` (prescriptive — what the system MUST do). The three-category model itself is described in the READMEs `$trace:init` copies into place; scaffold assumes those READMEs are there.
...
Resolve the scope's durable-context folder per [docs-folder-resolution.md](../../shared/docs-folder-resolution.md). The folder must contain `system/README.md` and `architecture/README.md` (placed by `$trace:init`). If either is missing, tell the developer to run `$trace:init` first (pass the same path argument when the scope isn't cwd) and stop — scaffold relies on the canonical structure being in place.
...
- `$trace:distil` keeps `system/` and `architecture/` files current as work happens.
...
- This is a one-time bootstrap. Ongoing capture is `$trace:distil`'s job.
