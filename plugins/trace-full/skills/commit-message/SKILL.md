...
- **Invoked with a commit-ish argument** (`$trace-full:commit-message HEAD~2`, `$trace-full:commit-message abc1234`): use `git show <ref>` and `git log -1 --format=%B <ref>` to read the diff and any existing message.
...
- Single commit only. For a whole PR or squash-merge message, use `$trace-full:pr-description`.
