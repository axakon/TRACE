...
- **With a PR number** (`$trace-full:pr-description 123`): run `gh pr view 123 --json baseRefName,headRefName,number,title,body`, fetch the base if needed, and diff against `origin/<base>`.
- **With a base branch** (`$trace-full:pr-description main`): diff `<base>...HEAD`.
...
If the change made a substantial decision with no ADR in the diff, say so in your final message so the developer can run `$trace-full:adr`. Do not invent an ADR entry.
...
Without `gh` or `glab`, the PR-number and apply paths are unavailable, and base inference falls back to git. For a single commit, use `$trace-full:commit-message`.
