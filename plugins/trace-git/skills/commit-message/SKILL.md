...
- **With a commit-ish** (`$trace-git:commit-message HEAD~2`): read `git show <ref>` and `git log -1 --format=%B <ref>`.
...
Check the draft against the rules files and fix it before showing it. Output the message in a fenced block. If the developer asked only for the text, stop there. If they asked you to commit, commit with this message. For a PR or squash-merge message, use `$trace-git:pr-description`.
